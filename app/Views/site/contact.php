    ...
    <form class="contact__form" method="post" action="<?= url('/contact') ?>">
        ...
        <?php if ($selectedPackage): ?>
            <div class="package-pick package-pick--selected">
                ...
                <span class="package-pick__price"><?= e(($selectedPackage['price_label'] ?: 'From') . ' ' . money((float) $selectedPackage['price_from'], (string) $selectedPackage['currency'])) ?></span>
                ...
                <select name="package_id" onchange="this.form.package_slug.value=''">
                    ...
                    <?php foreach ($packages as $package): ?>
                        <option value="<?= (int) $package['id'] ?>" <?= $picked === (int) $package['id'] ? 'selected' : '' ?>>
                            <?= e($package['name']) ?> (<?= e(($package['price_label'] ?: 'From') . ' ' . money((float) $package['price_from'], (string) $package['currency'])) ?>)
                        </option>
                    <?php endforeach; ?>
                </select>
            </label>
        ...
        <label class="field">
            ...
            <input type="text" name="budget" value="<?= e((string) old('budget', $selectedPackage ? (string) (int) $selectedPackage['price_from'] : '')) ?>" placeholder="e.g. 400000">
        </label>
